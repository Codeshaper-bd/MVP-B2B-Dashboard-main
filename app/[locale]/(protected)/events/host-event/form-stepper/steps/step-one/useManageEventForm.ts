...
function useManageEventForm() {
  ...
  const isGratuity = useWatch({
    control: formContextProps.control,
    name: "eventDetails.isGratuity",
    defaultValue: defaultValues.eventDetails.isGratuity,
  });
  const hideGuestlist = useWatch({
    control: formContextProps.control,
    name: "eventDetails.hideGuestlist",
    defaultValue: defaultValues.eventDetails.hideGuestlist,
  });
  const isCrowdMeterEnabled = useWatch({
    control: formContextProps.control,
    name: "eventDetails.isCrowdMeterEnabled",
    defaultValue: defaultValues.eventDetails.isCrowdMeterEnabled,
  });
  const isRecurring = useWatch({
    control: formContextProps.control,
    name: "eventDetails.isRecurring",
    defaultValue: defaultValues.eventDetails.isRecurring,
  });
  const recurringFor = useWatch({
    control: formContextProps.control,
    name: "eventDetails.recurringFor",
    defaultValue: defaultValues.eventDetails.recurringFor,
  });
