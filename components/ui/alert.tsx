...
const alertVariants = cva(
  cn(
    ...
    "[&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground [&>svg~*]:pl-7 [&>svg]:text-inherit",
  ),
  {
    variants: {
      color: {
...
>(({ className, ...props }, ref) => (
  <h5
    ref={ref}
    className={cn(
      "mb-2 grow text-lg font-medium leading-none tracking-tight text-inherit",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "flex grow items-center gap-1.5 text-sm [&_p]:leading-relaxed",
      className,
    )}
    {...props}
  />
