import React from "react";

function BatteryIcon({ ...props }: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      viewBox="0 0 25 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        opacity="0.35"
        d="M0.5 3.00065C0.5 1.80403 1.47005 0.833984 2.66667 0.833984H19.3333C20.53 0.833984 21.5 1.80403 21.5 3.00065V9.00065C21.5 10.1973 20.53 11.1673 19.3333 11.1673H2.66667C1.47005 11.1673 0.5 10.1973 0.5 9.00065V3.00065Z"
        stroke="currentColor"
      />
      <path
        opacity="0.4"
        d="M23 4.00098V8.00098C23.8047 7.6622 24.328 6.87411 24.328 6.00098C24.328 5.12784 23.8047 4.33975 23 4.00098Z"
        fill="currentColor"
      />
      <path
        d="M2 3.66732C2 2.93094 2.59695 2.33398 3.33333 2.33398H18.6667C19.403 2.33398 20 2.93094 20 3.66732V8.33398C20 9.07036 19.403 9.66732 18.6667 9.66732H3.33333C2.59695 9.66732 2 9.07036 2 8.33398V3.66732Z"
        fill="currentColor"
      />
    </svg>
  );
}

export default BatteryIcon;
