...
function ImagePreview({ file, id, onClick, index }: ImagePreviewProps) {
  ...
  return (
    <div className="relative h-[59px] w-[79px] shrink-0 rounded-[var(--radius-md,8px)] border border-solid border-[#333741] bg-[#161B26]">
      ...
      <button
        type="button"
        className="absolute right-[-4.33px] top-[-6px] flex h-fit w-fit cursor-pointer items-center justify-center rounded-full bg-warning p-[5px]"
        onClick={handleClick({ file, id, index, onClick })}
      >
        <CrossIcon className="size-1.5 shrink-0 text-white" />
      </button>
    </div>
  );
}
