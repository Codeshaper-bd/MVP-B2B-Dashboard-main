...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-0 z-[100] flex max-h-screen w-full flex-col-reverse sm:bottom-auto sm:end-4 sm:top-4 sm:flex-col md:max-w-[400px]",
      className,
    )}
    {...props}
  />
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border border-default-200 bg-secondary text-foreground",
        loading: "border border-default-200 bg-secondary text-foreground",
        success: "border border-default-200 bg-default-50 text-foreground",
        error: "border border-default-200 bg-default-50 text-foreground",
        destructive:
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute end-2.5 top-2.5 rounded-md p-1 text-[#85888E] transition-opacity focus:text-primary focus:outline-none focus:ring-2 focus:ring-primary",
      className,
    )}
    toast-close=""
    {...props}
  >
    <XIcon className="size-2.5 text-inherit" />
  </ToastPrimitives.Close>
));
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-sm font-normal leading-5 text-default-700", className)}
    {...props}
  />
));
