...
function DashboardEventCard({ item }: { item: TEventSummary }) {
  const router = useRouter();
  const { name, address, startTime, price, media, slug } = item;
  const isEventCompany = useIsEventCompany();
  const dashboardUpcomingLink = isEventCompany
    ? `/event-company/events/upcoming-events/${slug}`
    : `/events/upcoming-events/${slug}`;
  return (
    <Card
      className="group cursor-pointer overflow-hidden rounded-lg bg-secondary shadow-none hover:border-primary"
      onClick={() => router.push(dashboardUpcomingLink)}
    >
