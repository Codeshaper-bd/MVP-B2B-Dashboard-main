...
import { prepareTicketingFormStateData } from "./utils/form-state/ticketing";
import {
  handleCreateEvent,
...
export const useEventStepperForm = () => {
  ...
  const [pendingUpdateData, setPendingUpdateData] =
    useState<Partial<TCreateAnEventArgs> | null>(null);
  ...
  const handleUpdateAnEvent = async (step: number): Promise<boolean> => {
    try {
      const { eventDetails, ticketing, challenges, promotions } = getValues();
      switch (step) {
        case 0: {
        ...
        case 1: {
          const addonData: TRelationInput[] | undefined =
            ticketing?.addons?.map(
              (addon): TRelationInput => ({
                id: addon.id,
              }),
            ) ?? [];

          let guestListData: TGuestListConditionalType | null = null;

          if (ticketing?.isFreeGuestList) {
            guestListData = {
              isFreeGuestList: !!ticketing?.isFreeGuestList,
              guestListLimit: ticketing?.guestListLimit,
              guestListLimitType: ticketing?.guestListLimitType,
              perUserGuestListLimitQty: ticketing?.perUserGuestListLimitQty,
            };
          } else {
            guestListData = null;
            guestListData = {
          ...
          const updateAnEventRes = await updateAnEvent({
            slug: eventSlug,
            body: guestListData as unknown as Partial<TCreateAnEventArgs>,
          }).unwrap();

          if (!updateAnEventRes?.success) {
            throw new Error(
              updateAnEventRes?.message || "Failed to update event information",
            );
          }
          ...
          if (!updateAnEventRelationRes?.success) {
            throw new Error(
              updateAnEventRelationRes?.message ||
                "Failed to update addon & event relation",
            );
          }

          const addonLocallyRemoveRes =
            await localStorageUtil.removeItemAsync("addons");

          if (!addonLocallyRemoveRes?.success) {
            throw new Error(
              addonLocallyRemoveRes?.message ||
                "Failed to remove addon from local storage",
            );
          }
  ...
  const handleNext = async (): Promise<boolean> => {
    try {
      const { eventDetails, ticketing, table, challenges, promotions } =
        getValues();
  ...
  useEffect(() => {
    const iife = async () => {
      if (
        ...
        /* step 1 start */
        const { ticketing } = await prepareTicketingFormStateData({
          getAnEventData,
        });

        setValue("ticketing", ticketing);
        /* step 1 end */
